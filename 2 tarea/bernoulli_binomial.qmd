...
-   
```{r}
#| label: exr-bernoulli-maquina-solucion-directa
#| code-fold: true
...
cat("a) P(no defectuosa):", 1 - p1, "\n")

# b) Media y Varianza de Bernoulli
cat("b) Media (mu):", p1, "\n")
cat("b) Varianza (sigma^2):", p1 * (1 - p1), "\n")

# c) P(X=2) en Bin(10, 0.02)
cat("c) P(X=2) en Bin(10, 0.02):", 
    round(dbinom(x = 2, size = n1, prob = p1), 6), "\n")
```
...
-   
```{r}
#| label: exr-examen-solucion-directo
#| code-fold: true

# 1. Definición del Parámetro de Éxito (p)
p <- 0.25 # P(acertar) = 1/4

cat("\n--- Análisis de la Distribución Bernoulli (Examen) ---\n")

# a) Modelo de la variable aleatoria
cat("a) Modelo de la V.A.: X ~ Bernoulli(p =", p, ")\n")

# b) Probabilidad de acertar (P(X=1))
cat("b) Probabilidad de acertar (p):", p, "\n")

# c) Parámetros estadísticos (cálculo directo)
# Media E(X) = p
cat("c) Media E(X):", p, "\n")

# Varianza Var(X) = p * (1 - p)
cat("c) Varianza Var(X):", p * (1 - p), "\n")
...
:::
```{r}
#| label: exr-binomial-monedas-solucion-simple
...
prob_a <- dbinom(x = 8, size = n, prob = p)
cat("a) P(X=8):", prob_a, "\n")


# b) Probabilidad de obtener 12 o más caras (P(X >= 12))
# (Versión simple: sumar las probabilidades de 12, 13, 14 y 15)
k_valores <- 12:15 # Los casos que nos interesan
prob_b <- sum(dbinom(x = k_valores, size = n, prob = p))
cat("b) P(X>=12):", prob_b, "\n")
...
desv_est <- sqrt(n * p * (1 - p))

cat("c) Media:", media, "\n")
cat("c) Desviación Estándar:", desv_est, "\n")
```
...
```{r}
# Parámetros (igual que tu código)
N_pacientes <- 20
P_curacion <- 0.85

# --- Alternativa para b) P(15 <= X <= 19) ---

# 1. Crear un vector con todos los valores de k deseados
k_rango <- 15:19 # Esto crea el vector [15, 16, 17, 18, 19]

# 2. Calcular la probabilidad puntual (dbinom) para CADA valor en ese vector
prob_individuales <- dbinom(x = k_rango, size = N_pacientes, prob = P_curacion)

# 3. Sumar todas esas probabilidades individuales
probabilidad_b_alt <- sum(prob_individuales)

# El resultado es idéntico
cat("b) P(15 <= X <= 19) [Método Suma]:", round(probabilidad_b_alt, 4), "\n")
```
...
```{r}
#| label: exr-simulacion_5-solucion-simple
#| code-fold: true
#| fig-cap: "Verificación de la Propiedad Aditiva: Suma de Binomiales Z ~ Bin(12, 0.3) Simple"

# --- PARÁMETROS DE LA SIMULACIÓN (Igual) ---
...
# 1. Simulación de Variables Independientes (Igual)
set.seed(123) # ¡Esencial para la reproducibilidad!
# X ~ Bin(5, 0.3)
...
# 2. Parámetros Teóricos de Z (Igual)
n_z <- n1 + n2 # n + m = 12
p_z <- prob_exito # p = 0.3
media_z_teorica <- n_z * p_z # E(Z) = 12 * 0.3 = 3.6

# 3. Parámetros Observados (Igual)
media_z_observada <- mean(Z_sum) # Media empírica (será ~3.5976)

# 4. GRÁFICO SIMPLE (con Base R) y Comparación
# --- ESTA ES LA SECCIÓN SIMPLIFICADA ---

# 4a. Definir los límites de las barras del histograma
# Para que las barras se centren en 0, 1, 2... 12
breaks_hist <- seq(-0.5, n_z + 0.5, by = 1)

# 4b. Dibujar el histograma de la simulación (azul)
# 'freq = FALSE' es clave para que el eje Y sea 'Densidad' y no 'Frecuencia'
hist(Z_sum,
     breaks = breaks_hist,
     freq = FALSE, 
     col = "#1F77B4", # Azul (mismo color)
     border = "#004060", # Borde (mismo color)
     main = "Comprobación de Adición de Binomiales (N=10000)",
     sub = "Teoría (rojo) vs. Simulación (azul) para Z ~ Bin(12, 0.3)",
     xlab = "Número de Éxitos (k)",
     ylab = "Densidad Empírica / Probabilidad Teórica",
     ylim = c(0, 0.25) # Ajustar límite Y (la prob más alta es ~0.24)
)

# 4c. Calcular la PMF Teórica (la distribución objetivo)
K_teorico <- 0:n_z
PMF_teorica <- dbinom(K_teorico, size = n_z, prob = p_z)

# 4d. Superponer la distribución teórica (rojo)
# 'type = "h"' dibuja las líneas verticales (como geom_segment)
lines(K_teorico, PMF_teorica, type = "h", col = "#D62728", lwd = 3)
# 'points' dibuja los puntos (como geom_point)
points(K_teorico, PMF_teorica, col = "#D62728", pch = 16, cex = 1.5)

# 4e. Añadir una leyenda simple
legend("topright",
       legend = c("Simulación", "Teoría"),
       col = c("#1F77B4", "#D62728"),
       pch = c(15, 16), # 15 es un cuadrado, 16 es un círculo
       bty = "n" # sin caja
)

# 5. Resultados en consola (Exactamente igual que el original)
