...
-   
```{r}
#| label: exr-geom_1-solucion-directa
#| code-fold: true
...
# a) P(X=5) -> 4 fallos (k=4) antes del 1er éxito
prob_5_lanzamientos <- dgeom(x = 4, prob = p)
cat("P(X=5 lanzamientos):", round(prob_5_lanzamientos, 4), "\n")

# b) P(X > 3) -> 1 - P(X <= 3)
# P(X <= 3) significa 0, 1, o 2 fallos (k=0, 1, 2), que es pgeom(q=2)
prob_mas_de_3 <- 1 - pgeom(q = 2, prob = p)
cat("P(X>3 lanzamientos):", round(prob_mas_de_3, 4), "\n")

# c) Media y Varianza del número de lanzamientos (X)
# E(X) = 1/p
media_lanzamientos <- 1 / p
cat("Media_E(X):", media_lanzamientos, "\n")

# Var(X) = (1-p) / p^2
varianza_lanzamientos <- (1 - p) / (p^2)
cat("Varianza_Var(X):", varianza_lanzamientos, "\n")
```
...
analizar_inspeccion_maquina <- function(p) {
  ...
  cat("   Como los resultados son idénticos, la propiedad cumple. /n")
}
