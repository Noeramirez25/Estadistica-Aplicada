...
```{r}
#| label: exr-centro-atencion-solucion-directa
#| code-fold: true
...
cat("a) P(4 llamadas en 1 min):", 
    round(dpois(x = 4, lambda = lambda1), 4), "\n")

# b) P(X=0) en 30 segundos (lambda = 6 * 0.5 = 3)
cat("b) P(0 llamadas en 30 s) [lambda=3]:", 
    round(dpois(x = 0, lambda = lambda1 * 0.5), 4), "\n")

# c) Número esperado en 1 hora (lambda = 6 * 60 = 360)
cat("c) Número esperado en 1 hora:", lambda1 * 60, "\n")

# d) P(8 <= X <= 12) en 2 minutos (lambda = 6 * 2 = 12)
# P(X <= 12) - P(X <= 7)
cat("d) P(8 a 12 llamadas en 2 min) [lambda=12]:", 
    round(ppois(q = 12, lambda = lambda1 * 2) - ppois(q = 7, lambda = lambda1 * 2), 4), "\n")
```
...
```{r}
#| label: exr-hospital-solucion-descriptivo
#| code-fold: true
...
```{r}
#| label: exr-prop_aditiva-solucion-directa
#| code-fold: true
...
# --- 1. Simulación directa de todas las variables ---
# Simulamos cada variable por separado
X1 <- rpois(n_sim, lambda = lambdas_a_probar[1])
X2 <- rpois(n_sim, lambda = lambdas_a_probar[2])
X3 <- rpois(n_sim, lambda = lambdas_a_probar[3])
X4 <- rpois(n_sim, lambda = lambdas_a_probar[4])

# Sumamos todas las variables a la vez
W <- X1 + X2 + X3 + X4

# --- 2. Comparación Teórica vs. Observada (Directa) ---
# El lambda teórico es simplemente la suma de todos los lambdas
lambda_w_teorico <- sum(lambdas_a_probar)

cat("--- Verificación Aditiva para W = X1 + X2 + X3 + X4 ---\n")
cat(sprintf("Lambda Teórico (Σλ):        %.2f\n", lambda_w_teorico))
cat(sprintf("Media Observada de W:     %.4f\n", mean(W)))
cat(sprintf("Varianza Observada de W:  %.4f\n", var(W)))
cat("(La media y varianza observadas se aproximan al lambda total) ✅\n")
```
...
```{r}
#| label: exr-aplicacion_web-solucion-directa
#| code-fold: true

# --- Parámetros del Análisis ---
tasa_visitantes <- 15 # Tasa base (λ): 15 visitantes por minuto
...
```{r}
#| label: exr-inventarios-solucion-directa
#| code-fold: true
#| fig-cap: "Costo Esperado Diario vs. Nivel de Inventario (Q)"

# --- 1. Función de Ayuda (Separada, no anidada) ---
# Esta función calcula el costo total esperado para un nivel Q dado.
# Ahora es una función independiente y más fácil de entender.
calcular_costo_esperado <- function(Q, lambda, costo_mantener, costo_perdida, costo_adquisicion) {
...
# --- 2. Script Principal de Análisis (Lineal) ---
...
# c) Costo esperado para Q=12 (usamos la función de ayuda)
costo_q12 <- calcular_costo_esperado(Q = 12, lambda, costo_mantener, costo_perdida, costo_adquisicion)
cat(sprintf("c) Costo esperado total si Q=12: $%.2f\n", costo_q12))

# d) Nivel óptimo de inventario (Usando un bucle 'for' en lugar de 'sapply')
rango_q <- 1:20 # Rango razonable a evaluar
costos <- numeric(length(rango_q)) # Vector vacío para guardar los costos

# Bucle 'for' explícito: más fácil de leer
for (i in 1:length(rango_q)) {
  Q_actual <- rango_q[i]
  costos[i] <- calcular_costo_esperado(Q_actual, lambda, costo_mantener, costo_perdida, costo_adquisicion)
}

# Encontrar el costo mínimo y el Q óptimo (igual que antes)
...
# --- Gráfico de Costos (usando R base, sin cambios) ---
plot(rango_q, costos, 
