...
```{r ejemplo-calidad}
#| label: exr-binneg-solucion-simple
#| code-fold: true
...
```{r ejemplo-ventas}
#| label: exr-binneg-ventas-solucion-simple
#| code-fold: true
...
# 1. Número esperado de rechazos y Desviación Estándar
...
cat("1. Rechazos esperados (Teórico):\n")
cat("   ", round(rechazos_esperados, 2), "±", round(desv_std_rechazos, 2), "(Media ± DS)\n\n")

cat("2. P(10 ≤ rechazos ≤ 15) (Teórico):\n")
cat("   ", round(prob_10_15, 4), "\n")
```
...
```{r}
#| label: exr-binneg-ml-solucion-simple
#| code-fold: true
...
# Simulación del proceso (Igual que el original)
n_experimentos <- 10000
set.seed(123) # Añadido para reproducibilidad
...
# --- GRÁFICO SIMPLIFICADO (con Base R) ---

# Título y subtítulo
titulo_graf <- "Distribución de Iteraciones Fallidas en ML"
subtitulo_graf <- paste("p =", p_patron, ", r =", r_patrones,
                      ". Media =", round(media_iteraciones, 1))

# 1. Dibujar el histograma (freq=FALSE para mostrar Densidad)
hist(iteraciones_fallidas,
     ...
     main = titulo_graf,
     sub = subtitulo_graf,
     xlab = "Número de iteraciones fallidas",
     ylab = "Densidad"
)

# 2. Añadir la línea vertical para la media (como abline)
abline(v = media_iteraciones,
       col = "#E74C3C", # Color
       lty = "dashed", # Estilo de línea
       lwd = 3 # Grosor
)
```
...
```{r}
#| label: exr-baloncesto-solucion-simple
#| code-fold: true
...
# c) Simulación y comparación (sin cambios)
set.seed(2)
...
# 1. Definir los 'breaks' para que el histograma centre cada barra en un entero
max_fallos <- max(fallos_simulados)
breaks_hist <- seq(-0.5, max_fallos + 0.5, by = 1)

# 2. Dibujar el histograma (usando los datos simulados directamente)
# 'freq=TRUE' (el default) para mostrar Frecuencia Absoluta
hist_data <- hist(fallos_simulados,
     breaks = breaks_hist,
     col = "#0072B2",
     main = "Frecuencia de Fallos en Tiros Libres",
     sub = paste("Media Observada:", round(media_observada, 2)),
     xlab = "Número de Fallos (k)",
     ylab = "Frecuencia Absoluta"
)

# 3. Añadir las anotaciones (Texto y Flecha)
# Coordenadas (similares al original)
max_freq <- max(hist_data$counts)
x_label <- esperanza_fallos + 2
y_label <- max_freq * 0.8
label_txt <- paste("Media Teórica\nE(X) =", round(esperanza_fallos, 2))

# Añadir el texto (reemplaza a geom_text)
...
# Añadir la flecha (reemplaza a geom_segment con arrow)
arrows(x0 = x_label - 0.5, y0 = y_label,   # x0 = 3.42 + 1.5 = 4.92
       x1 = esperanza_fallos + 0.2, y1 = y_label, # x1 = 3.42 + 0.2 = 3.62
       col = "red",
       length = 0.15 # Tamaño de la punta de la flecha
)
```
...
```{r}
#| label: exr-biotecnologia-solucion-simple
#| code-fold: true
...
# Imprimir resultados directamente (sin data.frame)
cat("--- Resumen del Análisis de Pruebas de Biotecnología ---\n")

cat("Probabilidad de exactamente 12 fallas P(X=12):", round(prob_a3, 4), "\n")
cat("Probabilidad de más de 20 fallas P(X>20):", round(prob_b3, 4), "\n")
cat("Número esperado de fallas E(X):", round(esperanza_fallos_c, 2), "\n")

# Reemplazo de scales::dollar() con paste0() de base R
cat("Costo esperado de las fallas:", 
    paste0("$", round(costo_esperado, 2)), "\n")
```
