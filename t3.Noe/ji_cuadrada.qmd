...
```{r}
#| label: exr-chi-cuadrada-convergencia-tidy

# Cargar todas las librerías necesarias
library(ggplot2)
library(dplyr)
library(purrr)     # Para la función map_dbl
library(patchwork) # Para combinar gráficos de forma intuitiva

# --- 1. Función Tidyverse para Analizar la Convergencia ---
analizar_convergencia_tidy <- function(df, n_max = 10000) {
  
  # Medidas teóricas de la Chi-cuadrada
  mean_theoretical <- df
  ...
  simulation_df <- tibble(
    n = 1:n_max,
    sample_val = rchisq(n_max, df = df)
  ) %>%
    # --- 3. Cálculo de Estadísticos Acumulados ---
    mutate(
      # Media acumulada usando la función vectorizada cumsum
      cumulative_mean = cumsum(sample_val) / n,
      
      # Varianza acumulada usando purrr::map_dbl para iterar
      # Es más explícito y "tidy" que un bucle sapply
      cumulative_var = map_dbl(n, ~ if (.x < 2) NA else var(sample_val[1:.x]))
    )
  
  # --- 4. Crear Gráficos Individuales ---
  
  # Gráfico de convergencia de la media
  p_mean <- ggplot(simulation_df, aes(x = n, y = cumulative_mean)) +
    geom_line(color = "#3498DB", linewidth = 0.8) +
    geom_hline(yintercept = mean_theoretical, color = "#E74C3C", linetype = "dashed", linewidth = 1) +
    labs(
      title = "Convergencia de la Media",
      subtitle = paste("Valor Teórico (rojo):", mean_theoretical),
      x = "Tamaño de Muestra (n)", y = "Media Muestral"
    ) +
    theme_light() # Nuevo tema
    
  # Gráfico de convergencia de la varianza
  p_var <- ggplot(simulation_df, aes(x = n, y = cumulative_var)) +
    geom_line(color = "#2ECC71", linewidth = 0.8) +
    geom_hline(yintercept = var_theoretical, color = "#E74C3C", linetype = "dashed", linewidth = 1) +
    labs(
      title = "Convergencia de la Varianza",
      subtitle = paste("Valor Teórico (rojo):", var_theoretical),
      x = "Tamaño de Muestra (n)", y = "Varianza Muestral"
    ) +
    theme_light()

  # Histograma vs Densidad Teórica
  p_hist <- ggplot(simulation_df, aes(x = sample_val)) +
    geom_histogram(aes(y = after_stat(density)), bins = 50,
                   fill = "#9B59B6", color = "white", alpha = 0.8) + # Nuevo color
    stat_function(
      fun = dchisq, args = list(df = df),
      color = "#F1C40F", linewidth = 1.2 # Nuevo color
    ) +
    labs(
      title = "Histograma vs. Densidad Teórica",
      subtitle = paste("Muestra de", n_max, "observaciones"),
      x = "Valor Chi-cuadrada", y = "Densidad"
    ) +
    theme_light()
    
  # --- 5. Combinar y Mostrar los Gráficos con Patchwork ---
  # La sintaxis de patchwork es más simple y poderosa que grid.arrange
  final_plot <- (p_mean | p_var | p_hist) +
    plot_annotation(
      title = bquote(bold("Análisis de Convergencia para la Distribución Chi-cuadrada (χ²) con" ~ .(df) ~ "grados de libertad")),
...
analizar_convergencia_tidy(df = 5)
analizar_convergencia_tidy(df = 20)
analizar_convergencia_tidy(df = 50)
```
...
# --- 1. Parámetros ---
df <- 8             # Grados de libertad de la población Chi-cuadrada
n_sample <- 30      # Tamaño de cada muestra
n_sims <- 5000      # Número de simulaciones (medias muestrales)

# --- 2. Valores Teóricos (según el TCL) ---
# La media y DE de las medias muestrales
mean_theory <- df
sd_theory <- sqrt((2 * df) / n_sample)

# --- 3. Simulación ---
set.seed(456)
# Generamos 5000 medias muestrales. Cada media es de una muestra de tamaño 30.
sample_means <- map_dbl(1:n_sims, ~mean(rchisq(n_sample, df = df)))

# --- 4. Visualización ---
ggplot(data.frame(means = sample_means), aes(x = means)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "#2980B9", color = "white", alpha = 0.85) + # Tono azul
  # Curva Normal teórica que predice el TCL
  stat_function(
    fun = dnorm,
    args = list(mean = mean_theory, sd = sd_theory),
    color = "#E67E22", # Tono naranja
    linewidth = 1.3
  ) +
  labs(
    title = "Teorema Central del Límite en Acción",
    subtitle = bquote("Distribución de medias muestrales de una" ~ chi^2 ~ "(" ~ .(df) ~ ")"),
    x = "Media Muestral",
    y = "Densidad",
    caption = "La curva naranja es la Normal teórica predicha por el TCL."
  ) +
  theme_classic() + # Tema más limpio
...
#| label: exr-chi-cuadrada-aditividad-tidy
#| code-fold: true
...
df1 <- 4  # Grados de libertad 1
df2 <- 6  # Grados de libertad 2
n_sims <- 10000

# Grados de libertad de la suma teórica
df_sum <- df1 + df2
...
simulation_df <- tibble(
  # Generar muestra 1
  sample1 = rchisq(n_sims, df = df1),
  # Generar muestra 2
  sample2 = rchisq(n_sims, df = df2)
) %>%
  # Calcular la suma en una nueva columna
  mutate(
    sample_sum = sample1 + sample2
  )

# --- 3. Visualización ---
ggplot(simulation_df, aes(x = sample_sum)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50,
                 fill = "#F39C12", color = "#6E2C00", alpha = 0.8) + # Naranja/Marrón
  # Superponer la curva teórica de la Chi-cuadrada sumada
  stat_function(
    fun = dchisq,
    args = list(df = df_sum),
    color = "#3498DB", # Azul
  ...
  labs(
    title = "Propiedad de Aditividad de la Distribución Chi-cuadrada",
    subtitle = bquote("Suma de" ~ chi^2(.(df1)) + chi^2(.(df2)) ~ "se aproxima a" ~ chi^2(.(df_sum))),
    x = "Valor de la Suma Simulada",
    y = "Densidad",
    caption = bquote("La curva azul es la densidad teórica de" ~ chi^2(.(df_sum)) ~ ".")
  ) +
  theme_bw() + # Nuevo tema
  theme(
