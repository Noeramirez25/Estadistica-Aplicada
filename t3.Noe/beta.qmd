...
# --- PASO 1: Definir los objetos que faltaban ---

# Definimos el rango de valores para el eje x (la distribución Beta va de 0 a 1)
x <- seq(0.001, 0.999, by = 0.01)

# Creamos una lista que contendrá los pares de parámetros (a, b)
# Para que sea unimodal, ambos deben ser > 1
params_formas <- list(
  Unimodal = list(
    c(2, 5),    # a=2, b=5
    c(5, 2),    # a=5, b=2
...
# --- PASO 2: Tu código original (ahora funcionará) ---
...
graf_unimodal <- ggplot(df_unimodal, aes(x = x, y = density, color = parametros)) +
  ...
  labs(
    title = "Distribuciones Beta Unimodales",
...
```{r}
#| label: exr-simulacion_beta-solucion-tidy
#| code-fold: true
...
library(ggplot2)
library(patchwork)
library(purrr)     # Para la función map_dfr

# --------------------------------------------------------------------------
# 1. Definir Parámetros y Valores Teóricos
# --------------------------------------------------------------------------
alpha_beta <- 2
beta_beta <- 5
...
# Vector con los tamaños de muestra a simular
n_sizes_conv <- seq(100, 10000, by = 100)

# --------------------------------------------------------------------------
# 2. Realizar la Simulación (Reemplazo del bucle 'for')
# --------------------------------------------------------------------------
set.seed(1)
# Usamos map_dfr para iterar sobre cada tamaño de muestra 'n'.
# Para cada 'n', genera una muestra y devuelve una fila de data.frame
# con la media y la varianza. map_dfr une todas las filas al final.
resultados_conv <- map_dfr(n_sizes_conv, function(n) {
...
# --------------------------------------------------------------------------
# 3. Gráficos de Convergencia (código sin cambios)
# --------------------------------------------------------------------------
# Gráfica de convergencia de la Media
plot1 <- ggplot(resultados_conv, aes(x = n, y = Media_Muestral)) +
...
plot2 <- ggplot(resultados_conv, aes(x = n, y = Varianza_Muestral)) +
...
# --------------------------------------------------------------------------
# 4. Histograma de la Muestra más Grande (código sin cambios)
# --------------------------------------------------------------------------
...
plot3 <- ggplot(data_hist, aes(x = X)) +
  ...
  stat_function(
    fun = dbeta, args = list(shape1 = alpha_beta, shape2 = beta_beta),
    color = "red", linewidth = 1
  ) +
  labs(
    title = paste0("Distribución Beta(2, 5) con n=", n_max),
...
# --------------------------------------------------------------------------
# 5. Resumen Final (código sin cambios)
# --------------------------------------------------------------------------
...
```{r}
#| label: exr-tcl_beta-solucion-tidy
#| code-fold: true
#| fig-cap: "Verificación del Teorema Central del Límite (TCL) para Beta(2, 5)"
#| fig-width: 8
#| fig-height: 7
...
# --------------------------------------------------------------------------
# 1. Parámetros Iniciales (Necesarios del ejercicio anterior)
# --------------------------------------------------------------------------
alpha_beta <- 2
beta_beta <- 5
media_teorica <- alpha_beta / (alpha_beta + beta_beta)
...
k_sim_tcl <- 1000 # Número de simulaciones (medias muestrales)
n_sizes_tcl <- c(5, 15, 30, 60) # Tamaños de muestra

cat("--- Resultado Ejercicio 2: Teorema Central del Límite ---\n")

# --------------------------------------------------------------------------
# 2. Simulación y Creación de Gráficos (Reemplazo del Bucle)
# --------------------------------------------------------------------------
set.seed(42) # Se establece una sola semilla para la reproducibilidad de todo el bloque

# Creamos un tibble (un data.frame moderno) con los tamaños de muestra
# y luego añadimos columnas para las simulaciones y los gráficos.
tcl_results <- tibble(n = n_sizes_tcl) %>%
  # Para cada 'n', creamos una columna-lista que contiene las 'k' medias muestrales
  mutate(
    sim_means = map(n, ~replicate(k_sim_tcl, mean(rbeta(.x, alpha_beta, beta_beta))))
  ) %>%
  # Ahora, para cada 'n' y su set de medias, creamos un gráfico
  mutate(
    plot = pmap(list(sim_means, n), function(means_data, current_n) {
      
      # Parámetros teóricos de la distribución de la media muestral
      desv_std_teorica <- sqrt(varianza_teorica / current_n)
      
      # Creamos el gráfico
      ggplot(data.frame(Medias = means_data), aes(x = Medias)) +
        geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "lightblue", alpha = 0.7, color = "black") +
        stat_function(
          fun = dnorm, args = list(mean = media_teorica, sd = desv_std_teorica),
...
# --------------------------------------------------------------------------
# Calculamos las estadísticas observadas a partir de las simulaciones
tabla_tcl <- tcl_results %>%
  mutate(
    Media_Obs = map_dbl(sim_means, mean),
    Varianza_Obs = map_dbl(sim_means, var),
    Varianza_Teorica_Media = varianza_teorica / n
  ) %>%
  select(n, Media_Obs, Varianza_Obs, Varianza_Teorica_Media) # Seleccionamos solo las columnas de interés
...
# --------------------------------------------------------------------------
# 4. Combinar y Mostrar los Gráficos
# --------------------------------------------------------------------------
# Usamos patchwork para unir los gráficos que están en la columna 'plot'
plot_final_tcl <- wrap_plots(tcl_results$plot, ncol = 2) +
  plot_annotation(title = 'Teorema Central del Límite para Beta(2, 5)')
