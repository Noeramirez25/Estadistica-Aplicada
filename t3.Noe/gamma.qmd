...
```{r}
#| label: exr-TCL_gamma-solucion-tidy
#| code-fold: true
#| fig-cap: "Verificación del Teorema Central del Límite (TCL) para Gamma(2.5, 1.8)"
#| fig-width: 10
#| fig-height: 7
...
gamma_shape <- 2.5
gamma_rate <- 1.8
n_reps <- 1000 # Número de réplicas
sample_sizes <- c(5, 10, 20, 30, 50, 100) # Tamaños de muestra

# Parámetros Teóricos de la población Gamma
mean_theoretical <- gamma_shape / gamma_rate
var_theoretical <- gamma_shape / (gamma_rate^2)

cat("--- Resultado Ejercicio 1: TCL para Gamma (Versión Tidy) ---\n")
cat("Media Teórica (μ):", round(mean_theoretical, 4), "\n")

# Establecer una sola semilla para toda la simulación
set.seed(123)

# --- 2. Simulación y Creación de Gráficos (Enfoque Tidyverse) ---
tcl_data <- tibble(n = sample_sizes) %>%
  # 2a. Generar las simulaciones (crea una columna-lista)
  mutate(
    sim_means = map(n, ~replicate(n_reps, mean(rgamma(.x, shape = gamma_shape, rate = gamma_rate))))
  ) %>%
  # 2b. Calcular estadísticas observadas y teóricas
  mutate(
    Media_Obs = map_dbl(sim_means, mean),
    Varianza_Obs = map_dbl(sim_means, var),
    Varianza_Teorica_Media = var_theoretical / n,
    Desv_Std_Teorica_Media = sqrt(Varianza_Teorica_Media)
  ) %>%
  # 2c. Generar los gráficos (crea otra columna-lista)
  mutate(
    plot = pmap(list(sim_means, n, Desv_Std_Teorica_Media), 
                function(means_data, current_n, current_sd) {
      
      # Crear un data frame temporal para este gráfico específico
      data_plot <- data.frame(Medias = means_data)
      
      ggplot(data_plot, aes(x = Medias)) +
        geom_histogram(aes(y = after_stat(density)), bins = 30, 
                       fill = "#0072B2", color = "white", alpha = 0.8) + # Nuevo color azul
        stat_function(fun = dnorm, args = list(mean = mean_theoretical, sd = current_sd),
                      color = "#D55E00", linetype = "dashed", linewidth = 1.2) + # Nuevo color naranja
        labs(title = paste("n =", current_n), x = "Media Muestral", y = "Densidad") +
        theme_bw() + # Nuevo tema
        theme(plot.title = element_text(hjust = 0.5))
    })
  )
...
# Seleccionar solo las columnas para la tabla
tabla_tcl_tidy <- tcl_data %>%
  select(n, Media_Obs, Varianza_Obs, Varianza_Teorica_Media)

cat("\nTabla de Parámetros (Verificación TCL):\n")
print(tabla_tcl_tidy %>% kable(caption = "Comparación de Parámetros de Medias Muestrales", digits = 4) %>% kable_styling())

# --- 4. Mostrar Gráficos Combinados ---
# Accedemos a la columna-lista 'plot' que creamos
plot_final_tidy <- wrap_plots(tcl_data$plot, ncol = 3) + 
  plot_annotation(title = 'Verificación del Teorema Central del Límite para Gamma')

print(plot_final_tidy)
```
...
```{r}
#| label: exr-suma_exponenciales-solucion-tidy
#| code-fold: true
...
# --- 1. Parámetros de la Simulación ---
exp_rate <- 1.5
sum_sizes <- c(5, 10, 20) # Número de exponenciales a sumar
n_reps <- 1000            # Número de simulaciones (sumas)

cat("--- Resultado Ejercicio 2: Suma de Exponenciales (Versión Tidy) ---\n")

# Establecer semilla para reproducibilidad
set.seed(123)

# --- 2. Simulación y Creación de Gráficos en un solo Flujo ---
plot_data <- tibble(n_sum = sum_sizes) %>%
  # 2a. Para cada 'n_sum', generar las 'n_reps' sumas
  mutate(
    sim_sums = map(n_sum, ~replicate(n_reps, sum(rexp(.x, rate = exp_rate))))
  ) %>%
  # 2b. Para cada set de sumas, generar un gráfico
  mutate(
    plot = pmap(list(sim_sums, n_sum), function(sums_data, current_n) {
      
      # Data frame temporal para el gráfico
      df_plot <- tibble(Suma = sums_data)
      
      ggplot(df_plot, aes(x = Suma)) +
        geom_histogram(aes(y = after_stat(density)), bins = 40, 
                       fill = "#1ABC9C", color = "#145A32", alpha = 0.8) + # Tonos verdes
        stat_function(fun = dgamma, args = list(shape = current_n, rate = exp_rate),
                      color = "#E74C3C", linetype = "solid", linewidth = 1.2) + # Rojo sólido
        labs(
          title = paste("Suma de", current_n, "Exponenciales"), 
          subtitle = bquote(bold("Teórica:") ~ "Gamma(" * .(current_n) * ", " * .(exp_rate) * ")"),
...
# --- 3. Organizar y Mostrar los Gráficos ---
final_plot <- wrap_plots(plot_data$plot, ncol = 3) + 
  plot_annotation(title = 'Suma de Exponenciales vs. su Distribución Gamma Teórica')
