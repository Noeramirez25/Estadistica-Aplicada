...
#| label: exr-t-dist-analisis-tidy
#| code-fold: true


# --- 1. Parámetros de la Configuración ---
df_libertad <- 5 # Grados de libertad para la t de Student
sample_sizes <- c(100, 300, 1000) # Tamaños de muestra

# --- 2. Valores Teóricos ---
# Media teórica para t(df_libertad) es 0 (cuando df_libertad > 1)
mean_theoretical <- 0
# Varianza teórica para t(df_libertad) es df_libertad/(df_libertad-2) (cuando df_libertad > 2)
variance_theoretical <- df_libertad / (df_libertad - 2)

# --- 3. Calcular Estadísticos Muestrales (con purrr::map_dfr) ---
set.seed(123) # Nueva semilla para reproducibilidad

results_df <- map_dfr(sample_sizes, function(n_size) {
  sample_data <- rt(n_size, df = df_libertad)
  
  tibble(
    `Tamaño Muestra` = n_size,
    `Media Muestral` = mean(sample_data),
    `Media Teórica` = mean_theoretical,
    `Varianza Muestral` = var(sample_data),
    `Varianza Teórica` = variance_theoretical
  )
})

# Mostrar la tabla de comparación
kable(results_df, digits = 4, caption = "Estadísticos Muestrales vs. Teóricos para t(5)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)

# --- 4. Histograma para la muestra más grande (n=1000) ---
set.seed(42) # Manteniendo la semilla original para esta parte
large_sample <- rt(1000, df = df_libertad)

ggplot(data.frame(x = large_sample), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30,
                 fill = "#D4EEFF", color = "#404040", alpha = 0.8) + # Nuevo color de relleno y borde
  # Superponer la densidad teórica de t(5)
  stat_function(
    fun = dt, args = list(df = df_libertad),
    aes(color = "t(5) Teórica"), linewidth = 1.2 # Ligeramente más delgado
  ) +
  # Superponer la densidad de la normal estándar
  stat_function(
    fun = dnorm, args = list(mean = 0, sd = 1),
    aes(color = "Normal Estándar"), linewidth = 1.2, linetype = "dotted" # Nuevo tipo de línea
  ) +
  scale_color_manual(name = "Función de Densidad", 
                     values = c("t(5) Teórica" = "#E64B35FF", # Rojo coral
                                "Normal Estándar" = "#0072B5FF")) + # Azul profundo
  labs(
    title = "Comparación: Muestra t(5) vs. Distribuciones Teóricas", # Nuevo título
    subtitle = paste0("Muestra de n=1000. t(", df_libertad, ") tiene colas más pesadas que la N(0,1)."),
    x = "Valor de la Variable", # Nuevo nombre para el eje X
    y = "Probabilidad de Densidad" # Nuevo nombre para el eje Y
  ) +
  theme_classic() + # Nuevo tema de ggplot2
  theme(
...
datos_simulacion <- tibble(
  # Generar muestra de una Normal Estándar N(0,1)
  X = rnorm(n_sim, mean = 0, sd = 1),
  # Generar muestra de una Ji-cuadrada
  Y = rchisq(n_sim, df = grados_libertad)
) %>%
  # Calcular la variable T según la fórmula en una nueva columna
  mutate(
    T_calculada = X / sqrt(Y / grados_libertad)
  )

# --- 3. Comparar histograma con la densidad teórica de t(10) ---
ggplot(datos_simulacion, aes(x = T_calculada)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "#AED6F1", color = "#1B4F72", alpha = 0.8) + # Nuevos colores
  # Superponer la densidad teórica de t(10)
  stat_function(
    fun = dt,
    args = list(df = grados_libertad),
    color = "#E74C3C", # Rojo más vibrante
    linewidth = 1.3
  ) +
  labs(
    title = "Construcción de una Variable t de Student",
    subtitle = paste("T = X / sqrt(Y/n) donde X~N(0,1), Y~χ²(", grados_libertad, ")"),
    x = "Valor T Simulado",
    y = "Densidad de Probabilidad",
    caption = "La curva teórica t(10) se ajusta perfectamente a los datos simulados."
  ) +
  theme_bw() + # Nuevo tema
...
# --- 1. Parámetros de la simulación ---
n_reps <- 1000        # Número de réplicas o simulaciones
sample_n <- 10        # Tamaño de cada muestra individual
pop_mean <- 0         # Media poblacional (H₀)

# Grados de libertad para la distribución t resultante (n-1)
df_t <- sample_n - 1

# --- 2. Realizar la simulación con purrr::map_dbl ---
set.seed(789) # Para reproducibilidad

# map_dbl itera 1000 veces. En cada iteración, ejecuta el código
# y devuelve un único valor numérico (double).
t_values <- map_dbl(1:n_reps, ~{
  # a. Generar una muestra de una N(0,1)
  sample_data <- rnorm(sample_n, mean = pop_mean, sd = 1)
  
  # b. Calcular la media y desviación estándar de la muestra
  sample_mean <- mean(sample_data)
  sample_sd <- sd(sample_data)
  
  # c. Calcular y devolver el estadístico T
  (sample_mean - pop_mean) / (sample_sd / sqrt(sample_n))
})

# --- 3. Visualizar los resultados ---
# Crear un data frame para ggplot
results_df <- data.frame(t_stat = t_values)

ggplot(results_df, aes(x = t_stat)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "#5DADE2", color = "#154360", alpha = 0.8) + # Tonos de azul
  ...
  labs(
    title = "Distribución del Estadístico t bajo Simulación",
    subtitle = paste("Resultados de", n_reps, "muestras de tamaño", sample_n, "comparados con la curva teórica t(", df_t, ")"),
    x = "Valor del Estadístico t Simulado",
    y = "Densidad",
    caption = paste("La curva naranja representa la densidad teórica de t(", df_t, ").")
  ) +
